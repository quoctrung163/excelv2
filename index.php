...
if (isset($_GET["txtHoTen"]) || isset($_GET["txtNgaySinh"]) || isset($_GET["txtSoHieuVanBang"]) || isset($_GET["txtMaSoSinhVien"]) || isset($_GET["txtNamTotNghiep"])) {
  ...
  function searchByParams($name, $birthday, $no, $id, $year, $array)
  {
    ...
    if ($birthday != null) {
      $result = array_filter($result, function ($var) use ($birthday) {
        if (gettype($var[4]) == 'double') {
          $strChildConvertDateTime = PHPExcel_Style_NumberFormat::toFormattedString($var[4], 'dd/mm/yyyy');
  ...
  function printTable($array)
  {
    ...
    foreach ($array as $item) {
      ...
      echo '<td>' . $item[2] . '</td>';
      echo '<td>' . $item[3] . '</td>';
      echo '<td>' . PHPExcel_Style_NumberFormat::toFormattedString($item[4], 'dd/mm/yyyy') . '</td>';
      echo '<td>' . $item[5] . '</td>';
      echo '<td>' . $item[6] . '</td>';
      echo '</tr>';
    }
